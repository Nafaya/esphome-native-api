...
class Number extends Base {
    ...
    setState(state) {
        if (state < this.config.getMinValue()) throw new Error(`state(${state}) is less than the minimum(${this.config.getMinValue()})`);
        if (state > this.config.getMaxValue()) throw new Error(`state(${state}) is greater than the maximum(${this.config.getMaxValue()})`);
        this.command({ state });
    }
}
